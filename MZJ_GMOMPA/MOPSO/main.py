...
def main():
    ...
    # funct = int(input("1-ZDT2 | 2-Tanaka | 3-Osyczka | 4-Binh | 5-IGHS: "))
    funct = 5
    if funct == 1:
        cycle_ = 8000
    num_obj = 2
    ...
    np.savetxt("./coords417/mopso_in.txt", pareto_in)
    df_in = pd.DataFrame(pareto_in)
    df_in.to_csv("./coords417/mopso_in.csv", index=False)
    # Saving fitness value of pareto boundary
    np.savetxt("./coords417/mopso_fit.txt", pareto_fitness)
    df_fit = pd.DataFrame(pareto_fitness)
    df_fit.to_csv("./coords417/mopso_fit.csv", index=False)
