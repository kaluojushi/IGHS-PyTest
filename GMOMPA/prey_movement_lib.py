    ...
    for i in range(N):
        if use_FADs:
            ...
            if iter < max_iter / 3:
                ...
                R = get_uniform_random()
                step_size = RB * (Predator[i] - RB * Prey[i])
                Prey[i] = Prey[i] + P * R * step_size
            elif iter < max_iter * 2 / 3:
                if i < N / 2:
                    RL = get_levy_random()
                    R = get_uniform_random()
                    step_size = RL * (Predator[i] - RL * Prey[i])
                    Prey[i] = Prey[i] + P * R * step_size
                else:
                    RB = get_brownian_random()
                    step_size = RB * (RB * Predator[i] - Prey[i])
                    Prey[i] = Predator[i] + P * CF * step_size
            else:
                RL = get_levy_random()
                step_size = RL * (RL * Predator[i] - Prey[i])
                Prey[i] = Predator[i] + P * CF * step_size
    # print('!!!')
    # print(Prey)
