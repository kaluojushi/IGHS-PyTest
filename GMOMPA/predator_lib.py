...
# 输出的是多个 Pareto 前沿层
# 当前前沿层的解只被前一层的解所支配
def fast_non_dominated_sort(population, use_fitness=False, fitness=None):
    N = len(population)
    ...
    for i in range(N):
        ...
        for j in range(N):
            y = population[j]
            if dominates(x, y, use_fitness, fitness):
                set_of_x_dominates.append(j)
            elif dominates(y, x, use_fitness, fitness):
...
def sort_layer_using_crowding_distances(population, front_layer, use_fitness=False, fitness=None):
    front_fitness = np.array([fitness(x) for x in population[front_layer]]) if use_fitness else population[front_layer]
...
# 得到排序后的前沿层
def get_sorted_front_layers(population, front_layers=None, use_fitness=False, fitness=None):
    if front_layers is None:
        front_layers = fast_non_dominated_sort(population, use_fitness, fitness)
    print('front_layers:', front_layers)
    sorted_layers = [sort_layer_using_crowding_distances(population, front_layer, use_fitness, fitness) for front_layer
                     in front_layers]
...
# 通过选择前沿层中的解来获得新的种群
# 每个前沿层中的解按拥挤距离排序
def get_sorted_new_population(population, sorted_layers=None, use_fitness=False, fitness=None):
    if sorted_layers is None:
        sorted_layers = get_sorted_front_layers(population, use_fitness, fitness)
...
# 构造 Predator 种群
# 它是排序后种群中的前 N 个解
def construct_predator_population(sorted_population, first_N, N):
    if first_N >= N:
        return sorted_population[:N]
    else:
        return sorted_population[np.append(np.arange(first_N), np.random.randint(0, first_N, N - first_N))]


# Update the Archive population
# 更新 Archive 种群
def update_archive_population(Archive, Predator, max_arch, use_fitness=False, fitness=None):
    print('---update_archive_population start---')
    ...
    for i in range(len(Predator)):
        ...
        Archive_fitness = np.array([fitness(a) for a in Archive]) if use_fitness else Archive
        ...
        print('    start len(new_Archive): ', len(new_Archive))
        for j in range(len(new_Archive)):
            a = new_Archive[j]
            if dominates(s, a, use_fitness, fitness):
                sets_of_s_dominates.append(j)
            elif dominates(a, s, use_fitness, fitness):
                number_of_dominates_s += 1
            if epsilon_dominates(a, s, epsilon, use_fitness, fitness):
                number_of_epsilon_dominates_s += 1
        print('    sets_of_s_dominates: ', sets_of_s_dominates)
        print('    number_of_dominates_s: ', number_of_dominates_s)
        print('    number_of_epsilon_dominates_s: ', number_of_epsilon_dominates_s)
        ...
        if number_of_dominates_s != 0:
            print('    number_of_dominates_s != 0, continue')
            continue
        ...
        if len(sets_of_s_dominates) != 0:
            print('    len(sets_of_s_dominates) != 0, delete D and add s')
        ...
        # 步骤 4：否则，如果 ne != 0（即 Archive 中存在解能以 epsilon 支配解 s），则丢弃 s
            ...
            print('    number_of_epsilon_dominates_s != 0, continue')
            continue
        ...
        # 步骤 5：否则，如果 ne == 0（即 Archive 中不存在解能以 epsilon 支配解 s），则将 s 添加到 Archive
            ...
            print('    number_of_epsilon_dominates_s == 0, add s')
            new_Archive = np.concatenate([new_Archive, [s]])
        print('    now len(new_Archive): ', len(new_Archive))
    ...
    if len(new_Archive) > max_arch:
        print('  len(new_Archive) > N, shear_Archive')
        new_Archive = shear_Archive(new_Archive, max_arch, use_fitness, fitness)
    return new_Archive


# Shear Archive if its size is larger than max_arch
# 修剪 Archive，使其大小不超过 max_arch
def shear_Archive(Archive, max_arch, use_fitness=False, fitness=None):
    new_Archive = Archive.copy()
    ...
    def shear(Archive, e, use_fitness=False, fitness=None):
        new_Archive = []
        ...
        while len(origin_Archive) > 0:
            ...
            for b in origin_Archive:
                if epsilon_dominates(b, a, e, use_fitness, fitness):
    ...
    while len(new_Archive) > max_arch:
        new_Archive = shear(new_Archive, epsilon, use_fitness, fitness)
        epsilon *= 2
    return new_Archive
...
if __name__ == '__main__':
    ...
    front_layers2 = fast_non_dominated_sort(solutions2, use_fitness=True, fitness=fitness)
